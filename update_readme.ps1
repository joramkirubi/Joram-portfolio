...
orchestrator** (no LangGraph dependency — orchestration logic is implemented
...
```
         ...
         │  ┌────────────────────┐        ┌─────────────────────┐   │
         │  │   Triage Agent      │        │  Knowledge Agent     │   │
         │  │  category/urgency/  │        │  RAG: KB + similar   │   │
         │  │  risk classification│        │  past tickets        │   │
         │  └──────────┬───────────┘        └───────────┬──────────┘   │
         └─────────────┼─────────────────────────────────┼─────────────┘
                        │                                 │
                        └───────────────┬─────────────────┘
                          ...
                          ┌──────────────▼───────────────┐
                          │        Critic Agent            │◄──┐
                          │  reviews accuracy, policy,      │   │ reflection
                          │  tone → approve / reject         │   │ loop
                          └──────────────┬───────────────┘   │ (max N
                                 approved?│ no                │  retries)
                                         │ yes    └────────────┘
                          ┌──────────────▼───────────────┐
                          │      OUTPUT GUARDRAILS          │
                          │   PII scrub on outbound text    │
                          └──────────────┬───────────────┘
                                         │
                              ┌──────────▼───────────┐
                              │      Email Agent        │
                              │  customer reply and/or  │
                              │  internal escalation     │
                              └──────────┬───────────┘
                                         │
                              ┌──────────▼───────────┐
                              │   Long-Term Memory       │
                              │ (resolved ticket written │
                              │  back for future RAG)    │
                              └───────────────────────┘
```

The **Supervisor** (`orchestrator/supervisor.py`) owns this entire flow. No
agent ever calls another agent directly — every hand-off, retry, and routing
...
A linear pipeline (`triage → knowledge → resolution → critic → email`) would
force the Knowledge Agent to wait on the Triage Agent even though **neither
depends on the other's output** — both only need the raw customer message.
...
all of that evidence — a pattern used in real distributed reasoning and
multi-agent systems, rather than a single agent trying to do everything in
one pass.

The **reflection loop** (Critic → Resolution → Critic, bounded by
...
- **LLM layer** (`utils/llm_client.py`): if `GROQ_API_KEY` is unset (or the
  ...
  full pipeline — orchestration, tool calls, guardrails, memory, evals —
...
```
project-root/
├── agents/          Five specialized agents, each a pure function of TicketState
├── orchestrator/     Supervisor (fan-out/fan-in control flow) + shared TicketState
├── tools/             Mock backend APIs + email delivery tool
├── memory/            Short-term (in-process) and long-term (vector-indexed) memory
├── guardrails/        Input/output validation, PII detection & redaction, pre-action safety checks
├── evals/             Sample tickets + evaluation harness/metrics
├── utils/              LLM client (with offline fallback) + structured logger
├── config/             Settings (env-driven) + static knowledge base documents
├── main.py            CLI entrypoint (single ticket or --eval)
├── demo.py             Runs 7 curated scenarios showcasing every major behavior
├── api.py               FastAPI server + browser UI (static/index.html) for live testing
├── static/              Browser-based UI served by api.py
├── requirements.txt
├── .env.example
└── README.md          (this file)
```
...
### `config/`
- **`settings.py`** — Loads `.env` and exposes every tunable as a typed
...
- **`knowledge_base/*.txt`** — Five static policy documents (refunds,
  shipping, accounts, technical support, billing) ingested into the
  Knowledge Agent's vector store on first run.

### `orchestrator/`
- **`state.py`** — `TicketState`, the single dataclass passed between every
...
- **`supervisor.py`** — The orchestrator. Implements `process_ticket()`:
  runs input guardrails → fans out Triage + Knowledge in parallel via
  `ThreadPoolExecutor` → fans in to Resolution → runs the bounded
  Critic/reflection loop → runs output guardrails → hands off to the
...
### `agents/`
- **`triage_agent.py`** — Classifies `category` (billing/technical/refund/
  account/shipping/general), `urgency` (low/medium/high), and `risk_level`
  (low/medium/high) via a JSON-mode LLM call.
- **`knowledge_agent.py`** — Pure RAG: queries the static knowledge-base
...
- **`resolution_agent.py`** — The fan-in consumer. Builds a single prompt
...
- **`critic_agent.py`** — First runs a fast, deterministic guardrail check
...
- **`email_agent.py`** — Formats and "sends" (mock or real SMTP) the final
...
### `tools/`
- **`db.py`** — Lightweight SQLite-backed storage for orders, accounts,
...
- **`mock_apis.py`** — `check_order_status`, `issue_refund`,
  ...
  double-refunding).
- **`email_tool.py`** — `send_email()`. Mock mode (default) appends to
...
### `guardrails/`
- **`pii.py`** — Regex-based detection/redaction of emails, phone numbers,
  credit card numbers, and SSNs. Applied to both inbound messages and
  outbound responses.
- **`validators.py`** — `validate_input` (length/emptiness/high-risk
  ...
  (order-verification + auto-approval dollar limit) — the pre-action
  safety check specifically for the highest-risk tool call.

### `evals/`
- **`test_tickets.json`** — Six sample tickets with expected category,
  minimum urgency, and (for one) an expected human-review routing outcome.
- **`eval_runner.py`** — Runs every sample ticket through the real
...
### `utils/`
- **`llm_client.py`** — The only place that talks to Groq. Centralizes
  retries, JSON-mode prompting, and the offline mock fallback described in
  §1.3.
- **`logger.py`** — Structured JSON logging to `logs/system.log` (plus
  human-readable console output), and `agent_span()` — a context manager
  every agent uses to time itself and attach a structured trace entry
  (`agent`, `status`, `duration_ms`, `output`) to `TicketState.trace`.

### Root
- **`main.py`** — CLI: run one ticket (`--message`, `--customer`,
  `--order`) or the eval suite (`--eval`); `--json` dumps the full ticket
  state.
- **`demo.py`** — Seven scenarios in one script: a normal auto-resolved
...
- **`api.py`** — FastAPI server exposing the same `Supervisor` over HTTP
...
# Leaving it blank runs the system in offline mock-LLM mode (see §1.3) --
...
**already been refunded** (blocked to prevent a double refund) — printing
...
urgency, risk, final response, and full per-agent trace render live —
...
- **Tool execution lives in the Resolution Agent, not the LLM**: the LLM
  ...
  truth for whether something actually happened — the LLM's text can't
...
- **Business-logic guardrails, not just policy guardrails**: beyond the
  ...
  already refunded — a category of bug (double refunds) that a
...
git add README.md
git commit -m "Update README: document SQLite backend, new refund guardrail, web UI/API server, and correct demo scenario count"
git push
